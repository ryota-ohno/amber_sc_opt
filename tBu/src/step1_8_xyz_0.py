...
def main_process(args):
    auto_dir = f'/home/ohno/Working/amber_sc_opt/tBu/{args.auto_dir}'
    os.makedirs(auto_dir, exist_ok=True)
    os.makedirs(os.path.join(auto_dir,'amber'), exist_ok=True)
    os.makedirs(os.path.join(auto_dir,'gaussview'), exist_ok=True)
    amber_path=os.path.join(auto_dir,'amber')
    shutil.copy(f'/home/ohno/Working/amber_sc_opt/tBu/src/FF_calc.in',amber_path)
    shutil.copy(f'/home/ohno/Working/amber_sc_opt/tBu/monomer/{args.monomer_name}.frcmod',amber_path)
    
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path): 
        header = ['theta','A2','phi','a','b','z','E','E1','E2','E3','E4','status']
    ...
    if not os.path.exists(auto_csv_path1):
        header = ['theta','A2','phi','a','z','E1','status','file_name']
    ...
    if not os.path.exists(auto_csv_path2):
        header = ['theta','A2','phi','b','z','E2','status','file_name']
    ...
    if not os.path.exists(auto_csv_path3):        
        header = ['theta','A2','phi','a','b','z','E3','status','file_name']
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    auto_csv_mono = os.path.join(auto_dir,'step1_mono.csv');rows_mono = []
    with open(auto_csv_mono, mode='r', newline='') as f:
        reader = csv.DictReader(f)
        for row in reader:
            rows_mono.append(row)
    for idx, row in enumerate(rows_mono):
        ...
        with open(auto_csv_mono, mode='w', newline='') as f:
            ...
            writer.writeheader();writer.writerows(rows_mono)
    ...
    if len(dict_matrix)!=0:#終わりがまだ見えないなら
        for i in range(len(dict_matrix)):
            ...
            if not(alreadyCalculated):
                ...
                dictlist_Emono,fieldnames_mono = read_csv_to_dictlist(os.path.join(auto_dir,'step1_mono.csv'))
                dict_list_Emono_f=filter_dictlist(dictlist_Emono, params_dict_mono)
                if len(dict_list_Emono_f) == 0:
                    file_name = exec_gjf_mono(auto_dir, monomer_name, {**params_dict_mono},isTest=isTest)
                    new_dict_mono = {**params_dict_mono, 'Emono': '0.0', 'status': 'InProgress','file_name':file_name}
                    dictlist_Emono.append(new_dict_mono);write_dictlist_to_csv(os.path.join(auto_dir,'step1_mono.csv'), dictlist_Emono, fieldnames_mono)
