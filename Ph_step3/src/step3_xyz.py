...
def main_process(args):
    ...
    shutil.copy(f'/home/ohno/Working/amber_sc_opt/Ph_step3/src/FF_calc.in',amber_path)
    shutil.copy(f'/home/ohno/Working/amber_sc_opt/Ph_step3/monomer/benzene.frcmod',amber_path)
    ...
    if not os.path.exists(auto_csv_path): 
        ...
        with open(auto_csv_path, mode='w', newline='') as file:
            ...
            writer.writeheader()
    mono_file=f'/home/ohno/Working/amber_sc_opt/Ph_step3/monomer/{args.monomer_name}.out'
    E_mono=get_E(mono_file)[0]
    os.chdir(os.path.join(auto_dir,'amber'))
    isOver = False
    while not(isOver):
        #check
        isOver = listen(auto_dir,args.monomer_name,args.num_nodes,E_mono)##argsの中身を取る
        #time.sleep(0.1)

def listen(auto_dir,monomer_name,num_nodes,E_mono):##args自体を引数に取るか中身をばらして取るかの違い
    fixed_param_keys = ['theta','A2','a','b','z','phi'];opt_param_keys_1 = ['cx','cy','cz']
        ...
        for i in range(len(dict_matrix)):
            ...
            if not(alreadyCalculated):
                ...
                if len(dict_list_E1_f) == 0:
                    file_name = exec_gjf(auto_dir, monomer_name, {**params_dict1})
...
def final_process(args):
    auto_dir = f'/home/ohno/Working/amber_sc_opt/{args.monomer_name}_step3/{args.auto_dir}'
